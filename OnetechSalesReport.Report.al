report 50006 OnetechSalesReport
{
    DefaultLayout = RDLC;
    UsageCategory = Administration;
    RDLCLayout = './OnetechSalesReport_stf.rdl';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem(salesinvoiceline; "Sales Invoice Line")
        {
            ...
            column(RegistrationDate; format(RegistrationDate))
            {
            }
            column(PeriodEndday_; PeriodEndday)
            {
            }
            ...
            begin
                ...
                //SetFilter("Document No.", salesinvoiceline."Document No.");
                compinfo.get();
                if(showWithholdingtax = false) and (showfranchisetax = false)then begin
                    SetFilter("No.", '<>S0011|<>S0012|<>S0007|<>S0008|<>S0009|<>S0010');
                end
                else if(showfranchisetax = false)then begin
                        SetFilter("No.", '<>S0008|<>S0009|<>S0010');
                    end
                    else if(showWithholdingtax = false)then SetFilter("No.", '<>S0011|<>S0012|<>S0007');
            end;
            ...
            begin
                ...
                if(invheader."Currency Factor" > 0)then invGlEntryAmount:=salesinvoiceline.Amount / invheader."Currency Factor"
                else
                ...
                SalesMonthBegin:=0;
                SalesMonthEnd:=0;
                if("Period Start" <> 0D) and ("Period End" <> 0D)then begin
                    startmonth:=DATE2DMY("Period Start", 2);
                    endmonth:=DATE2DMY("Period End", 2);
                    daysperiod:="Period End" - "Period Start" + 1;
                    PeriodEndday:=DATE2DMY("Period End", 3);
                    if(startmonth = endmonth)then DayMonthEnd:=daysperiod
                    else
                        DayMonthEnd:=PeriodEndday;
                    if(startmonth = endmonth)then DaysMonthBegin:=0
                    else
                        DaysMonthBegin:=daysperiod - DayMonthEnd;
                    if(startmonth <> endmonth) and (daysperiod > 0)then SalesMonthBegin:=Amount * DaysMonthBegin / daysperiod
                    else
                        SalesMonthBegin:=0;
                    //Add Amount USD
                    if(invheader."Currency Factor" > 0)then SalesMonthBegin:=SalesMonthBegin / invheader."Currency Factor";
                    if(daysperiod > 0)then SalesMonthEnd:=Amount * DayMonthEnd / daysperiod
                    else
                        SalesMonthEnd:=0;
                    //Add Amount USD
                    if(invheader."Currency Factor" > 0)then SalesMonthEnd:=SalesMonthEnd / invheader."Currency Factor";
                end;
            end;
        }
        dataitem(salescrmemoline; "Sales Cr.Memo Line")
        {
            ...
            column(crmemopostingdate; Format(crmemoheader."Posting Date"))
            {
            }
            ...
            begin
                ...
                if crmemoheader.FindFirst()then begin
                    crmemosite:=crmemoheader.Site;
                    crmemoContract:=crmemoheader."Contract Code";
                    crSalesCustomerName:=crmemoheader."Sell-to Customer Name";
                end;
                if glaccount.Get("No.")then begin
                    invglaccountname:=glaccount.Name;
                end;
                if(crmemoheader."Currency Factor" > 0)then crmemoGlEntryAmount:=salescrmemoline.Amount / crmemoheader."Currency Factor"
                else
                ...
                crSalesMonthEnd:=0;
                if("Period Start" <> 0D) and ("Period End" <> 0D)then begin
                    crstartmonth:=DATE2DMY("Period Start", 2);
                    crendmonth:=DATE2DMY("Period End", 2);
                    crdaysperiod:="Period End" - "Period Start" + 1;
                    crPeriodEndday:=DATE2DMY("Period End", 1);
                    if(crstartmonth = crendmonth)then crDayMonthEnd:=crdaysperiod
                    else
                        crDayMonthEnd:=crPeriodEndday;
                    if(crstartmonth = crendmonth)then crDaysMonthBegin:=0
                    else
                        crDaysMonthBegin:=crdaysperiod - crDayMonthEnd;
                    if(crstartmonth <> crendmonth) and (crdaysperiod > 0)then crSalesMonthBegin:=Amount * crDaysMonthBegin / crdaysperiod
                    else
                        crSalesMonthBegin:=0;
                    //Add Onetech convert USD
                    if(crmemoheader."Currency Factor" > 0)then crSalesMonthBegin:=crSalesMonthBegin / crmemoheader."Currency Factor";
                    if(crdaysperiod > 0)then crSalesMonthEnd:=Amount * crDayMonthEnd / crdaysperiod
                    else
                        crSalesMonthEnd:=0;
                    //Add Onetech convert USD
                    if(crmemoheader."Currency Factor" > 0)then crSalesMonthEnd:=crSalesMonthEnd / crmemoheader."Currency Factor";
                end;
            end;
        }
    }
