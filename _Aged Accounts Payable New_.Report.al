report 50017 "Aged Accounts Payable New"
{
    DefaultLayout = RDLC;
    RDLCLayout = './ReportExt/Report 50017 Aged Accounts Payable.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'Antigüedad pagos JYM'; //'Aged Accounts Payable New'; Ok
    ...
    dataset
    {
        dataitem(Vendor; Vendor)
        {
            ...
            dataitem(CurrencyLoop; "Integer")
            {
                ...
                dataitem(TempVendortLedgEntryLoop; "Integer")
                {
                    ...
                    column(DocumentDescription; DocumentDescription)
                    {
                    }
                    column(OriginalDueDate; OriginalDueDate)
                    {
                    }
                    trigger OnAfterGetRecord()
                    var
                    ...
                    begin
                        ...
                        if UseExternalDocNo then DocumentNo:=VendorLedgEntryEndingDate."External Document No."
                        else
                        ...
                        DocumentDescription:='';
                        OriginalDueDate:=0D;
                        if VendorLedgEntryEndingDate."Document Type" = VendorLedgEntryEndingDate."Document Type"::"Credit Memo" then begin
                            if PurchCrm.Get(VendorLedgEntryEndingDate."Document No.")then begin
                                DocumentDescription:=PurchCrm."Posting Description";
                                OriginalDueDate:=PurchCrm."Due Date";
                            end;
                        end;
                        if VendorLedgEntryEndingDate."Document Type" = VendorLedgEntryEndingDate."Document Type"::Invoice then begin
                            if PurchInv.Get(VendorLedgEntryEndingDate."Document No.")then begin
                                DocumentDescription:=PurchInv."Posting Description";
                                OriginalDueDate:=PurchInv."Due Date";
                            end;
                        end;
                    end;
        ...
        dataitem(CurrencyTotals; "Integer")
        {
            ...
            begin
                ...
                if TempCurrencyAmount.FindSet(false, false)then repeat if TempCurrencyAmount.Date <> DMY2Date(31, 12, 9999)then AgedVendorLedgEntry[GetPeriodIndex(TempCurrencyAmount.Date)]."Remaining Amount":=TempCurrencyAmount.Amount
                        else
    ...
    begin
        ...
        if UseExternalDocNo then DocNoCaption:=ExternalDocumentNoCaptionLbl
        else
    ...
    begin
        ...
        //Dev: Amru 19/04/22
        if TempVendorLedgEntry.Get(VendorLedgEntry."Entry No.")then exit;
        TempVendorLedgEntry:=VendorLedgEntry;
        ...
        if TempCurrency.Get(TempVendorLedgEntry."Currency Code")then exit;
        if TempVendorLedgEntry."Currency Code" <> '' then Currency.Get(TempVendorLedgEntry."Currency Code")
        else
        begin
            Clear(Currency);
            Currency."Amount Rounding Precision":=GLSetup."Amount Rounding Precision";
    ...
    begin
        if Vendor.GetFilter("Global Dimension 1 Filter") <> '' then VendorLedgerEntry.SetFilter("Global Dimension 1 Code", Vendor.GetFilter("Global Dimension 1 Filter"));
        if Vendor.GetFilter("Global Dimension 2 Filter") <> '' then VendorLedgerEntry.SetFilter("Global Dimension 2 Code", Vendor.GetFilter("Global Dimension 2 Filter"));
    end;
}
