report 50018 "CUSTOMERS TRIAL BALANCE"
{
    // version NAVW17.10
    DefaultLayout = RDLC;
    RDLCLayout = './ReportExt/Report 50018 CUSTOMERS TRIAL BALANCE.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'CUSTOMERS TRIAL BALANCE'; //Ok
    ...
    dataset
    {
        dataitem(DataItem23; Customer)
        {
            ...
            dataitem(DataItem1; "Cust. Ledger Entry")
            {
                ...
                begin
                    beforeperiod:=FALSE;
                    IF("Posting Date" < fromdate)THEN beforeperiod:=TRUE;
    ...
    trigger OnPreReport();
    begin
        IF(fromdate = 0D) OR (todate = 0D)THEN ERROR('Plotesoni datat!');
    end;
    var fromdate: Date;
    todate: Date;
